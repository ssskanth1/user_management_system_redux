...
const AdminHome = () => {
    ...
    const imageUrl = '../../../public/admin.jpg';
    return (
    ...
    <div className="flex justify-between items-center py-4">
      ...
      <div className="flex items-center">
        ...
        <div className="flex items-center">
          ...
          <button className='mx-10 inline-block rounded border-2 border-danger px-6 pb-[6px] pt-2 text-xs font-medium uppercase leading-normal text-danger transition duration-150 ease-in-out hover:border-danger-600 hover:bg-neutral-500 hover:bg-opacity-10 hover:text-black focus:border-danger-600 focus:text-danger-600 focus:outline-none focus:ring-0 active:border-danger-700 active:text-danger-700 dark:hover:bg-neutral-100 dark:hover:bg-opacity-10"' 
          id="profileBtn" onClick={logOut} >Log out</button>
        </div>
      </div>
    </div>
  </div>
      ...
      <div id="wrapper" className="min-h-screen flex flex-col">
        <div className="container mx-auto px-4">
          {!edit ? (
            <>
              <header id="header" className="flex-grow">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {users && users.length > 0 && users.map((user, index) => (
                    <tr key={user.id}>
                      ...
                      <td className='px-4 py-2 text-center'> <img src={user.profile?process.env.REACT_APP_BASIC_URL + 'Profile/' + user.profile:process.env.REACT_APP_BASIC_URL + 'Profile/' + 'download.jpg'} className="h-10 w-10" alt="DP" /> </td>
                      ...
                      <td className='px-4 py-2 text-center'>
                        <div className="flex justify-center">
                          <button className='bg-blue-500 text-white p-2 px-4 m-auto rounded w-auto' onClick={() => { setEdit(users[index]) }} >Edit</button>
                          <button className='bg-green-500 text-white p-2 m-auto rounded w-auto' onClick={() => blockUser(user._id, index)}>{user.status ? 'Block' : 'Unblock'}</button>
    ...
    <div className="flex-grow flex items-center justify-center">
        <div className="bg-opacity-40 bg-slate-50 p-8 rounded shadow-md w-full max-w-md">
          <div className="flex items-center justify-center mb-4">
            <img src={show ? show : (process.env.REACT_APP_BASIC_URL + 'Profile/' + edit.profile)} className="h-24 w-24 rounded-full object-cover mr-4 " alt="" />
          ...
          <div className="flex justify-end">
            <button className='bg-blue-500 text-white p-2 rounded' onClick={updateUser}>Update</button>
